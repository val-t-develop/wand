...
int main (int argc, char **argv) {
    vector<Path> pathes = Path::getCurrentDir().getDirContent();
    for (auto path : pathes) {

        if (isdigit(static_cast<unsigned char>(path.getFilename().c_str()[0]))) {
            string n = path.getFilename();
            string cmd = "cd "+ path.getName()+ " && sh "+path.getName()+"/test.sh && cd ..";
            system(cmd.c_str());
            if (Path(path.getName()+"/test.result").readFile().starts_with("FAIL")) {
                std::cout << "Test #" << n << " is FAIL\n";
            }
        }
    }
    return 0;
